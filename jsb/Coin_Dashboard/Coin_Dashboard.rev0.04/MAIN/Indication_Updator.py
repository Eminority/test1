...
def process_indicators_update(conn, table_name):
    try:
        ...
        query = f"""
            SELECT KOREAN_NAME, MARKET, CANDLE_DATE_TIME_UTC, HIGH_PRICE, LOW_PRICE, TRADE_PRICE, 
        ...
        # 종목별로 데이터 처리
        grouped = df.groupby('KOREAN_NAME')
        result_list = []
        ...
        for name, group in grouped:
            ...
            # 지표 계산
            group['EMA_15'] = cal_ema(group['VWAP'], 5)
            group['EMA_360'] = cal_ema(group['VWAP'], 20)

            if len(group['VWAP']) >= 20:
                group['BALL_HIGH'], group['BALL_LOW'] = cal_ball(group['VWAP'], window=20, num_std_dev=2)
            else:
                group['BALL_HIGH'], group['BALL_LOW'] = pd.NA, pd.NA

            group['RSI_360'] = cal_rsi(group['VWAP'], period=5)

            sto_df = cal_sto(group[['HIGH_PRICE', 'LOW_PRICE', 'TRADE_PRICE']], period=20, smooth_k=3, smooth_d=3)
