...
def fetch_10days_data(market):
    ...
    target_time = end_time - timedelta(days=10)
...
def fetch_latest_data(market):
    try:
        ...
        if response.status_code == 200:
            return pd.DataFrame(response.json()).drop_duplicates(subset=['candle_date_time_utc'])
        elif response.status_code == 429:
            print("[WARN] Too many requests. Waiting for 1 minute...")
            time.sleep(60)
        else:
...
def update_db_with_data(conn, data, market):
    ...
    WHEN NOT MATCHED THEN
        INSERT (market, candle_date_time_utc, candle_date_time_kst, opening_price, high_price, low_price, trade_price, volume)
        VALUES (:market, :timestamp, :kst_time, :opening_price, :high_price, :low_price, :trade_price, :volume)
    """
...
def archive_old_data(conn):
    ...
    archive_query = """
    INSERT INTO upbit_minute_data_archive (market, candle_date_time_utc, candle_date_time_kst, opening_price, high_price, low_price, trade_price, volume)
    SELECT market, candle_date_time_utc, candle_date_time_kst, opening_price, high_price, low_price, trade_price, volume
...
def initialize_db(conn, markets):
    ...
    for market in markets:
        ...
        if not data.empty:
            data = data[['candle_date_time_utc', 'candle_date_time_kst', 'opening_price', 'high_price', 'low_price', 'trade_price', 'candle_acc_trade_volume']]
